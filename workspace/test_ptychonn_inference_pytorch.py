...
if __name__ == '__main__':
    ...
    data = np.load('data/ptychonn/20191008_39_diff_sample.npy')
    data = data.reshape([data.shape[0] * data.shape[1], *data.shape[2:]])
    logger.info(data.shape)

    inferencer = pppc.inferencer.PyTorchInferencer(config)
    inferencer.build()
    pred_amp, pred_phase = inferencer.batch_infer(data)

    fig, axes = plt.subplots(2, 2)
    for i in range(pred_phase.shape[0]):
        axes[i // 2][i % 2].imshow(pred_phase[i, 0])
    fig.suptitle('Phase')
    fig, axes = plt.subplots(2, 2)
    for i in range(pred_amp.shape[0]):
        axes[i // 2][i % 2].imshow(pred_amp[i, 0])
